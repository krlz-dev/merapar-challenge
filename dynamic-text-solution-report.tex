...
\pagestyle{fancy}
\fancyhf{}
\rhead{Dynamic Text Solution}
\lhead{Carlos A. Rojas}
\cfoot{\thepage}
...
\title{
    \vspace{-1.5cm}
    {\Huge\bfseries Dynamic Text Solution}\\
    \vspace{0.3cm}
    {\Large Technical Analysis and Implementation Report}\\
    \vspace{0.2cm}
    {\large Merapar Company Challenge}
}

\author{
    \textbf{Carlos Andres Monserrat Rojas Rojas}\\
    \textit{Software Engineer}\\
    \href{mailto:your.email@example.com}{carlos.rojas@example.com}
}
...
\subsection{Observer Pattern (SSR Solution)}
SSE implementation manages client connections with real-time broadcasting:
\begin{itemize}[leftmargin=*]
    \item \texttt{SSEService} maintains active client connections
    \item Content updates trigger broadcast to all observers
    \item Automatic cleanup of disconnected clients
\end{itemize}

\subsection{Strategy Pattern}
Two distinct update strategies for different requirements:
\begin{itemize}[leftmargin=*]
    \item \textbf{Polling:} Static solution with periodic HTTP requests
    \item \textbf{Push:} SSR solution with Server-Sent Events
\end{itemize}
...
\subsection{CDK Implementation}
Both solutions use AWS CDK v2 with TypeScript:
...
\section{Key Insights and Future Enhancements}

\subsection{Technical Insights}
\begin{itemize}[leftmargin=*]
    \item \textbf{Constraint Impact:} Simple requirements eliminated most conventional web patterns
    \item \textbf{Cost vs. Capability:} 70x cost difference for functionally equivalent solutions
    \item \textbf{Architecture Trade-offs:} Minimal complexity vs. real-time capabilities
\end{itemize}

\subsection{Future Enhancements and Solution Selection}
\textbf{The choice between solutions depends entirely on project requirements:}

\subsubsection{When to Choose the S3 Static Solution}
If the project is defined as a simple content display with infrequent updates, the S3 solution is more than sufficient:
\begin{itemize}[leftmargin=*]
    \item Basic content updates (weekly/monthly)
    \item Single admin user
    \item Cost-sensitive projects
    \item Simple deployment requirements
\end{itemize}

\subsubsection{When to Choose the SSR Platform Solution}
If the project is defined as a platform requiring robust functionality, proper design patterns become essential:
\begin{itemize}[leftmargin=*]
    \item Frequent content updates (multiple times daily)
    \item Multiple admin users requiring real-time collaboration
    \item Integration with other systems
    \item Scalability and performance requirements
    \item Enterprise-grade monitoring and security
\end{itemize}

\textbf{Key Insight:} Clear project definition drives architecture choice. Without understanding the actual use case, frequency of updates, user base, and growth expectations, any solution recommendation would be premature.
